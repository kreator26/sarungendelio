/* ════════════════════════════════════════════════════════════
   SARUNG ENDE — Statistik Jangkauan Pengunjung (dari Firestore)
================================================================ */
...
async function muatStatistik() {
  ...
  document.getElementById('top-halaman').innerHTML = top.map(([h, c]) => `
    <div>
    ...
    </div>`).join('') || '<p class="text-sm text-cocoa/50">Belum ada data kunjungan.</p>';

  /* ── Tabel kunjungan terbaru ── */
  const terbaru = data.filter(x => x.waktu)
    .sort((a, b) => b.waktu.toDate() - a.waktu.toDate()).slice(0, 10);

  document.getElementById('tabel-kunjungan').innerHTML = terbaru.map(x => `
    ...
    </tr>`).join('') || '<tr><td colspan="4" class="py-3 px-3 text-cocoa/50">Belum ada kunjungan.</td></tr>';

  showToast('✅ Statistik berhasil dimuat.');
}
